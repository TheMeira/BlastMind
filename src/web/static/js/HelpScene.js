class HelpScene extends Phaser.Scene {
    ...
    aiLines() {
        return [
            ...
            { type: 'body',    text: 'Searches every ordering and position for the current three-piece hand and immediately picks the highest-scoring move by a heuristic function that weighs factors such as cells placed, lines cleared, and board compactness. Efficient and consistent, but has no lookahead beyond the known hand — it cannot sacrifice short-term score for a better long-term board state.' },
            { type: 'rule' },
            { type: 'heading', text: 'Beam Search' },
            { type: 'body',    text: 'Searches every ordering and position for the current three-piece hand, then performs a forward search over the K most promising resulting boards (the beam width), sampling several simulated future hands to see which choice holds up best several turns ahead. Combining exhaustive search of the known hand with genuine lookahead into the unknown future makes this the strongest agent in the project, at the cost of the most computation per move.' },
            { type: 'rule' },
            ...
            { type: 'body',    text: 'Monte Carlo Tree Search first solves the current three-piece hand exactly via exhaustive search, then spends its full simulation budget exploring what happens afterward, using chance nodes to model the random draw of each new hand. Each iteration selects nodes via the UCB1 formula to balance exploration and exploitation, then simulates forward with a heuristic-guided rollout policy to estimate the outcome. The move sequence with the highest visit count is chosen. Combines exact tactical search for the known hand with statistical lookahead into the unknown future, using the same board-evaluation heuristic as the Greedy agent.' },
        ];
    }
